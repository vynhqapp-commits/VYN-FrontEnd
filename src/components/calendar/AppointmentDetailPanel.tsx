...
export default function AppointmentDetailPanel({
  ...
  const clientName = appointment.Client?.full_name ?? appointment.client_id ?? appointment.customer_id ?? '—';
  const serviceName =
    appointment.Service?.name ?? appointment.service_id ?? appointment.services?.[0]?.service?.name ?? '—';
  const staffName =
    (appointment.Staff && appointment.Staff.name) ||
    appointment.staff?.name ||
    (appointment as any).staff_id ||
    '—';
  const locationName =
    appointment.branch?.name ||
    appointment.Location?.name ||
    (appointment as any).branch_id ||
    '—';
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-salon-sand/30">
          <div>
            <h2 className="font-display text-base font-semibold text-salon-espresso">Appointment</h2>
            <p className="text-xs text-salon-stone mt-1">Status: {statusLabel}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="rounded-2xl border border-salon-sand/40 bg-salon-cream/30 p-4 space-y-3">
            <div className="flex items-center justify-between gap-2">
              <p className="text-sm font-semibold text-salon-espresso">Update status</p>
              <p className="text-xs text-salon-stone">{changingId === appointment.id ? 'Updating…' : ''}</p>
            </div>
          ...
          <div className="text-xs text-salon-stone">
            Tip: You can also update statuses from the Calendar view by selecting another status.
          </div>
        </div>
      </div>
    </div>
